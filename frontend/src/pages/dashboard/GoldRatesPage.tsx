...
export function GoldRatesPage() {
  ...
  const load = async () => {
    ...
    const [currentResponse, historyResponse] = await Promise.all([
      ...
      http.get(`/shops/${selectedShopId}/gold-rates/history`, { params: { page: 1, limit: 20 } }),
    ]);
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-slate-500">Update daily carat rates used by pricing and old-gold exchange.</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <Card title="Current Rate">
          <pre className="overflow-auto rounded-lg bg-slate-950 p-3 text-xs text-emerald-200">
            {current ? JSON.stringify(current, null, 2) : 'No current rate available.'}
          </pre>
        </Card>
      </div>
            ...
            <tbody>
              {history.map((entry) => (
                <tr key={entry.id} className="border-t border-slate-200">
                  <td className="px-2 py-2">{new Date(entry.effectiveDate).toLocaleString()}</td>
