...
export function ReportsPage() {
  ...
  const loadDaily = async () => {
    ...
    const response = await http.get(`/shops/${selectedShopId}/reports/daily-closing`, { params: { date } });
    setDaily(unwrap(response));
  };

  const loadSummary = async () => {
    if (!selectedShopId) return;
    const [summaryResponse, inventoryResponse, goldResponse] = await Promise.all([
      http.get(`/shops/${selectedShopId}/reports/sales-summary`, { params: { from, to } }),
      http.get(`/shops/${selectedShopId}/reports/inventory-value`),
      http.get(`/shops/${selectedShopId}/reports/gold-sold`, { params: { from, to } }),
    ]);
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-slate-500">Generate daily closing, sales summary, inventory value, and gold sold reports.</p>
      </div>
        ...
        <Card title="Range Reports">
          ...
          <div className="mt-3 grid grid-cols-1 gap-3">
            <pre className="overflow-auto rounded-lg bg-slate-950 p-3 text-xs text-amber-200">
              {summary ? JSON.stringify(summary, null, 2) : 'Sales summary pending.'}
            </pre>
            <pre className="overflow-auto rounded-lg bg-slate-950 p-3 text-xs text-sky-200">
              {inventoryValue ? JSON.stringify(inventoryValue, null, 2) : 'Inventory value pending.'}
            </pre>
            <pre className="overflow-auto rounded-lg bg-slate-950 p-3 text-xs text-fuchsia-200">
              {goldSold ? JSON.stringify(goldSold, null, 2) : 'Gold sold report pending.'}
            </pre>
          </div>
        </Card>
      </div>
