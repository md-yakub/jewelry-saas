...
export function CustomOrdersPage() {
  ...
  const load = async () => {
    ...
    const [customerResponse, craftsmanResponse, ordersResponse] = await Promise.all([
      http.get(`/shops/${selectedShopId}/customers`, { params: { page: 1, limit: 100 } }),
      http.get(`/shops/${selectedShopId}/craftsmen`),
      http.get(`/shops/${selectedShopId}/custom-orders`, { params: { page: 1, limit: 50 } }),
    ]);
  ...
  const updateStatus = async (id: string, status: string) => {
    if (!selectedShopId) return;
    await http.patch(`/shops/${selectedShopId}/custom-orders/${id}/status`, { status });
    await load();
  };
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-slate-500">Track bespoke designs from intake to delivery.</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
        <Card title="Create Order" className="xl:col-span-1">
          <form className="space-y-3" onSubmit={handleSubmit(onSubmit)}>
            ...
            <Input type="number" step="0.001" placeholder="Estimated weight" {...register('estimatedWeight')} />
            <Input type="number" step="0.01" placeholder="Advance payment" {...register('advancePayment')} />
            ...
            <Button type="submit" disabled={isSubmitting}>{isSubmitting ? 'Saving...' : 'Create Order'}</Button>
          </form>
        </Card>
            ...
            {orders.map((order) => (
              <div key={order.id} className="rounded-lg border border-slate-200 p-3 text-sm">
                <div className="flex items-center justify-between gap-2">
                  <p className="font-semibold">{order.customer?.name ?? 'Customer'} - {order.estimatedWeight} g</p>
                  <Badge value={order.status} />
                </div>
                <p className="mt-1 text-slate-600">{order.designNotes}</p>
                <div className="mt-2 flex flex-wrap gap-2">
                  {['PENDING', 'DESIGN_CONFIRMED', 'IN_PROGRESS', 'READY', 'DELIVERED', 'CANCELLED'].map((status) => (
                    <button
                      key={status}
